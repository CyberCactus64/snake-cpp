...
void update_game_matrix(int &length, Directions direction, Position &head_pos) { // per sincronizzare la matrice snake e quella del gioco
    for (unsigned int y = 0; y < HEIGHT; ++y) {
        for (unsigned int x = 0; x < WIDTH; ++x) {
            if (snake_matrix[y][x] == ' ') {
                game_matrix[y][x] = ' ';
...
void move_snake(int &length, Directions direction, Position &head_pos) {
    /* if (direction == Up && !touch_border(head_pos)) {
        --head_pos.y;
        update_game_matrix(length, direction, head_pos);
    }
    ...
    for (unsigned int y = 0; y < HEIGHT; ++y) {
        for (unsigned int x = 0; x < WIDTH; ++x) {
            if (direction == Up) {
                for (int y_up = 0; y_up < length; ++y_up) {
                    snake_matrix[head_pos.y-1][head_pos.x];
                }
            }
            if (direction == Down) {
                for (int y_down = 0; y_down < length; ++y_down) {
                    snake_matrix[head_pos.y+1][head_pos.x];
                }  
            }
            if (direction == Right) {
                for (int x_right = 0; x_right < length; ++x_right) {
                    snake_matrix[head_pos.y][head_pos.x+1];
                }  
            }
            if (direction == Left) {
                for (int x_left = 0; x_left < length; ++x_left) {
                    snake_matrix[head_pos.y-1][head_pos.x-1];
                }   
            }
        }
    }
